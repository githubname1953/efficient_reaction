...
the filament location and the last level of the tree is a vector of subunits. This is necessary for implementing 
...
struct cbt
{
    ...
    {
        int pos = dq[0];
        ll[pos].loc = ll[ll.size()-1].loc;
        dq[ll[pos].loc-lo]=pos;
    ...
    void transport(int &n, int &state, boost::circular_buffer<int> &dq)
    {
        ...
        next=ll[j].loc-lo+1;
        update1(j,-1);
        if(next!=dq.size()) update1(dq[next],1);
        else state = 0;
    }
